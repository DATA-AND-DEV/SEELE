...
O [ADR 0045](adr/0045-toda-versao-continua-de-pe.md) decidiu que o app vira
...
> **Os números dos ADRs citados aqui são os desta base, e eles mudam na
> integração.** Este trabalho foi escrito sobre o ramo dos MODs, onde a decisão
> do launcher é a **0045** e a dos MODs é a **0044**. Na `main` as duas são
> **0046** e **0045**: um ADR próprio da `main` — o 0044, sobre o portão da
> subida medida — entrou antes das duas e deslocou a numeração em um. O texto
> da decisão do launcher é **idêntico** nas duas bases; o que mudou foi só o
> número, e o próprio ADR já registra a renumeração na primeira página.
>
> Isto não é previsão: conferido na `main` desta máquina, que já traz
> `docs/adr/0046-toda-versao-continua-de-pe.md` e
> `docs/adr/0045-mods-o-produto-base-tem-regras-e-um-mod-nao.md`. A `main`
> também já contém o commit sobre o qual este trabalho foi escrito, então o que
> falta é trazer **este** trabalho para lá — e a renumeração é o que ele ainda
> não tem.
>
> Quem integrar isto troca `0045` por `0046` e `0044` por `0045` nesta página,
> nas referências de `crates/seele-lancador` e na linha do filtro de log em
> `apps/seele-app/src/main.rs`. Está escrito aqui porque um número de ADR errado
> aponta com confiança para a decisão de outra pessoa, que é pior que não
> apontar.
...
| onde | o que diz |
|---|---|
| ADR 0045 | a lista de revogação assinada fica **dentro do manifesto de versões** |
| ADR 0045 | «A mesma peça serve à remoção de um MOD do indexador, e as duas devem ser a mesma coisa — duas listas de revogação seriam duas chances de esquecer uma» |
| ADR 0044 | o catálogo do indexador é assinado com uma chave **separada** da do atualizador, porque «uma chave que atesta duas coisas deixa as duas se passarem uma pela outra, e a do 0026 autoriza instalar programa» |
...
- **Uma implementação**: `crates/seele-lancador/src/revogacao.rs`. Um formato,
  ...
  o ADR 0045 comprou ao dizer «a mesma peça»: o custo de esquecer uma das duas
...
- **Dois documentos, cada um na casa do seu assunto e assinado pela chave que
  ...
  | MODs removidos | dentro do catálogo do indexador | a do indexador (ADR 0044) |
...
O ADR 0045 disse «duas listas de revogação seriam duas chances de esquecer
...
instalações antigas que o ADR 0045 promete manter de pé. Então mudança aditiva
...
|---|---|---|---|
| 1 | `latest.json` descreve uma versão, não todas | `empacotar/manifesto.py`, que é o único lugar onde a regra do manifesto mora, e o `release.yml` que o chama | o seletor de versão tem uma opção só, e o ADR 0045 vale no código e não no produto |
...
| 4 | não há `unit` (protocolo e API de MOD) por versão | precisa sair do build: são `seele_proto::version::PROTOCOL_VERSION` e o número da API de `api/v1.json` | a conferência de coerência do ADR 0045 não acontece, e a defesa volta a ser só o `negotiate` — que é justamente o que o ADR tirou do posto principal |
...
hospedar e entrar. Está no ADR 0045 e está no código: a lista é conferida em
