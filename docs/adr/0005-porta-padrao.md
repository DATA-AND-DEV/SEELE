...
Status: proposto
Contexto: `specs/01-arquitetura.md` marca "8383 [EM ABERTO: confirmar]" e `specs/04-servidor-seele.md` repete `escuta = "0.0.0.0:8383"`. Mas o protótipo entregue em `design/` mostra `seele://toquio-3.dogma.central:7743` na tela `02 AUTENTICAÇÃO`. Duas fontes de verdade divergem.
...
Consequências: baixas em qualquer direção. O que não pode acontecer é a divergência sobreviver até a porta entrar em documentação de usuário ou em `seeled.toml` de exemplo.
