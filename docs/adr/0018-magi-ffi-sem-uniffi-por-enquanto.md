# 0018 — `magi-ffi` com a forma que o `uniffi` exige, sem a dependência ainda

Status: aceito

Contexto: `specs/06-clientes-gui.md` descreve a superfície da `magi-ffi` e diz que ela deve ser "gerada com `uniffi` sempre que possível". Ao chegar em M5 a pergunta ficou concreta: o `uniffi` entra agora, com o desktop, ou em M6, com o mobile?
...
Decisão: escrever a `magi-ffi` **com a forma que o `uniffi` exige**, sem depender do `uniffi`. A dependência e as anotações entram em M6, junto com o primeiro consumidor de binding.
...
- Tipos de valor (`Snapshot` e o que ele contém) usam `u64`, `u32`, `String`, `bool`, `Vec` e enums — **nunca** os newtypes de `magi-proto`. Um `PilotId` atravessando a fronteira é conhecimento de protocolo vazando para a casca, que é o que `specs/06` proíbe em uma frase: "Se o frontend precisa saber o que é um `ssrc`, algo está errado."
