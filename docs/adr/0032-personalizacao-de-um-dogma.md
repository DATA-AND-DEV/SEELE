# ADR 0032 — Personalização de um Dogma: o nome no fio, a cor numa placa, e nenhuma imagem por enquanto
...
Ele **compartilha uma pergunta** com o ADR 0031 — o que pertence a um Dogma e o
que pertence a esta máquina — e a resposta está lá, escrita uma vez. O que este
acrescenta é o outro lado: o que um Dogma pode dizer sobre si mesmo, e até onde
...
encarar — que **um MOD não acompanha um Dogma**: a sala recomenda, e instalar
continua sendo um ato.

**A diferença que muda tudo é uma frase: a escolha é de outra pessoa.** Um MOD
você instala, numa tela sua, com as medidas na frente. A cor de um Dogma chega
...
- **O nome já viaja e já é desenhado.** `ServerMessage::Session` carrega
  `dogma: String` (`crates/seele-proto/src/control.rs:909`), conferido contra
  `MAX_CLIENT_NAME_LEN` = 64 (`control.rs:61`, `control.rs:1391`);
  `seele_ffi::Snapshot` o repassa como `dogma`
  (`crates/seele-ffi/src/types.rs:557`); e as duas cascas o escrevem no
  cabeçalho. Esta metade está pronta.
- **A outra metade não existe: não há como nomear um Dogma pela janela.**
  `DogmaConfig::name` é campo de struct montada no `main.rs`
  (`crates/seele-server/src/lib.rs:57-58`), e o botão HOSPEDAR AQUI passa a
  string literal `"Casa"` — `apps/seele-app/src/main.rs:372-376`. Todo Dogma
  hospedado pelo app do mundo se chama Casa.
- **Existe onde guardar.** A tabela `configuracao`
  (`crates/seele-server/src/casper/schema.rs:175`) nasceu na migração 2 para o
  ADR 0021 e é descrita como «configuração do Dogma que não cabe num arquivo,
  ...
  (`casper/attachments.rs:61`), e ele vale igual aqui.
...
- **O vermelho é exclusivo de alerta e de queda**, e os guardas que o protegem
  ...
  São quatro em `apps/seele-app/tests/frontend.rs` mais
  `alert_and_accent_stay_distinct_in_sixteen_colours` em
  `crates/seele-tui/src/theme.rs`. O ADR 0029 mediu o resto e achou o número
...
- **O quadro de controle tem teto de 16 KiB.** `MAX_FRAME_LEN`
  (`crates/seele-proto/src/control.rs:49`), e o `Session` já carrega Cages,
  Linhas, papéis e permissões dentro dele.
- **O que esta máquina lembra de cada Dogma já tem arquivo.**
  `crates/seele-core/src/conhecidos.rs`: uma linha por Dogma, com endereço,
  apelido, último Cage e quando foi a última visita. O cabeçalho do módulo diz
...
**O nome é do Dogma e viaja no fio, como já viaja. A cor é uma escolha dentro de
...
- **Quem escreve:** quem hospeda, na mesma tela em que o ADR 0030 pôs a portaria,
  e pelo mesmo caminho — acessor no `Hospedagem`, falando direto com o CASPER da
  máquina, **sem verbo novo de protocolo**. O argumento do 0030 vale sem
  emenda: nomear o Dogma é decisão de quem tem o arquivo dele na mão, e expor
...
- **Renomear com o Dogma no ar precisa de um aviso**, ou quem está dentro
...
- **O teto é o que já existe:** 64 bytes, conferido no `control.rs`, e a mesma
  ...
  (`a_blank_name_never_leaves_the_dogma_inside_a_session`).
...
### Um Dogma não repinta a janela de quem entra
...
O 0029 recusou que um MOD acompanhasse um Dogma por três motivos, e os três
...
medidas, e aí instalava. Uma cor de Dogma não tem tela, não tem arquivo, não tem
diferença para ler: ela chega no `Session` junto com a lista de Cages. Se a
...
Então a regra é a mesma, e mais estreita: **um Dogma nunca dá valor a um token,
...
O que um Dogma personaliza é **o botão dele na trilha** — 56 px no comp v3, o
...
mudar a hierarquia da tela às três da manhã. **A pergunta «um Dogma pode repintar
...
Dogma caberia seria a janela inteira, e aí a resposta teria de ser não e ponto —
...
| Dogma (este) | outra pessoa | **um nome, e só** | ninguém, porque não há o que medir | — |

Um MOD pode declarar valor porque quem instala escolheu, e porque há uma tela em
que a medida é lida por alguém antes de valer. Um Dogma declara **nome**, de uma
...
**Estado vence identidade na placa.** Uma placa cujo Dogma caiu **perde a cor do
Dogma** e assume a forma de queda, que é vermelha e é do produto. Não há um
...
A metade que devolve a decisão a quem olha: **a cor que um Dogma escolhe é uma
...
Dogma onde já esteve, e que o próprio módulo descreve como a conveniência que
«pode ser apagado sem consequência».

É o «recomendar, e instalar continua sendo um ato» do ADR 0029, na menor escala
em que ele cabe: o Dogma sugere, a máquina guarda a escolha de quem está na
...
**Um Dogma não manda imagem.** É a recusa mais dura desta página, e ela tem três
razões que se somam:
...
anexo é nome e tamanho. Uma imagem que um Dogma empurra é **o mesmo gênero de
...
o `Session` já carrega Cages, Linhas, papéis e permissões. Um ícone dentro dele
disputaria espaço com o conteúdo da sala — e um Dogma grande que deixasse de
...
Dogma por nós, deterministicamente. O comp v3 já a usa — `TÓQUIO-3` vira `T3`
...
- **Teto pequeno e declarado:** um quadrado, **128×128 e 16 KiB**, recusado por
  ...
  56 px em tela de alta densidade e para que o pior caso de cache por Dogma seja
...
Ele pode ser construído hoje, sozinho, e melhora o produto de hoje: todo Dogma
...
outra pessoa. A pergunta «um Dogma pode repintar a janela alheia» tem duas
respostas conforme o que existe na tela: com uma sessão por vez, a única
resposta é não, porque a janela inteira é aquele Dogma; com a trilha do 0031, a
...
1. **O Dogma declara valores hexadecimais, e o produto mede na conexão como o
...
2. **O Dogma acompanha um MOD inteiro.** Já recusada pelo ADR 0029, na seção «Um
   MOD não acompanha um Dogma», com os motivos inteiros. Este ADR não a reabre;
...
3. **Nada: um Dogma é o nome dele.** É o que existe, é de graça e funciona.
...
4. **A pessoa pinta as placas, e o Dogma não opina.** Segura, e quase adotada. O
   que ela perde é o único caso que o pedido descreve — quem hospeda quer que o
   Dogma dele **seja** de um jeito, e a placa é onde isso apareceria para os
   outros. **Ela não foi recusada: virou a segunda metade da decisão**, como
   preferência local que vence a proposta do Dogma. Adotar só ela seria responder
...
- **O literal `"Casa"` sai do `main.rs`**, e todo Dogma hospedado pelo botão
...
- **Um evento novo no fim da enumeração**, se renomear com o Dogma no ar for
...
- **Um campo novo no `Session`**, também no fim, para o nome de cor. É um
  ...
  Dogma que escolhe uma cor que este cliente não conhece desenha a placa padrão.
...
achar um Dogma pela placa laranja acha o próximo Dogma laranja do mesmo jeito.
Um Dogma hostil que escolhe a cor de um Dogma conhecido é uma imitação barata, e
...
disso nem deve haver: quem entra num Dogma escolheu entrar. O que o produto pode
...
ele, uma variante que Dogmas velhos nunca emitem.
