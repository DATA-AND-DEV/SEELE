...
### Task 5: O protocolo — a v4 e as quatro mensagens
...
### Task 6: O servidor guarda quem empresta, e escolhe (burramente) quem serve quem
...
**Interfaces:**
- Consumes: `ClientMessage::EmprestarSubida`, `ServerMessage::{SirvaTelaPara, AssistaTelaPor}` (Task 5)
- Produces:
...
### Task 7: O cliente pede ao par, e cai para o servidor quando falha
...
**Interfaces:**
- Consumes: `ligar`, `ParLigado`, `ErroDePar` (Tasks 1–4); `ServerMessage::AssistaTelaPor` (Task 5)
...
### Task 8: Quem empresta repassa os bytes, por pedaço e a partir de um quadro-chave
...
### Task 9: A costura, num servidor de verdade com dois clientes
...
> as Tasks 6, 7 e 8 fixarem, e escrevê-lo aqui de antemão seria inventar nomes que
...
Escreva os corpos. O que faltar de ligação nas Tasks 6, 7 e 8 aparece aqui — é para isso que este teste vem depois delas e não antes.
...
### Task 10: O roteiro de duas máquinas mede o que nenhum teste daqui mede
...
O **subprojeto B** só deve ser desenhado depois de a Task 10 ter sido executada
