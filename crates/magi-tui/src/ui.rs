...
//! [`crate::app`] or in `magi-core` and has leaked.
//!
...
/// The outer frame: `MAGI ─ 同期率 ─ 第3新東京市 ─ 12:04:33`.
fn title_block(app: &App, theme: Theme) -> Block<'static> {
    let mut spans = vec![
        Span::styled(" MAGI ", theme.accent()),
...
fn render_bar(frame: &mut Frame<'_>, app: &App, theme: Theme, area: Rect) {
        ...
        .enumerate()
    {
        ...
        let style = if index == 0 {
            theme.sync(magi_core::SyncBand::of(app.bar.sync))
        } else {
            theme.body()
        };
...
pub fn bar_segments(bar: &Bar, at_field: bool, total_isolation: bool) -> Vec<String> {
    let mut segments = vec![
        format!(
            "SYNC {}{:>3}%",
            Theme::sync_mark(magi_core::SyncBand::of(bar.sync)),
            bar.sync
        ),
...
fn render_boot(frame: &mut Frame<'_>, app: &App, theme: Theme, area: Rect) {
    ...
    let lines = vec![
        Line::from(Span::styled("  MAGI SYSTEM", theme.accent())),
        Line::from(""),
