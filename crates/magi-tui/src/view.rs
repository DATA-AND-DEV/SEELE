...
//! `magi_core::state`: flattening a tree into rows, turning a server timestamp
...
use magi_core::{AlertSeverity, DisconnectReason, Link, Room};
...
mod tests {
    ...
    fn telemetry_lands_in_the_bar() {
        let mut room = room();
        room.apply(&ServerMessage::Telemetry(magi_core::Telemetry {
            rtt_ms: 38.0,
            jitter_ms: 12.0,
            loss_fraction: 0.002,
            subsystems: Vec::new(),
