...
//! the translation from crossterm events into [`magi_tui::app::Key`]. Every
//! decision it makes about *what* to do lives in [`magi_tui::app`]; every
//! decision about the session lives in `magi-core`. If this file grows a
...
///
/// `$MAGI_HOME` first, so a second client on one machine can be a second pilot
...
fn config_dir() -> PathBuf {
    ...
    if let Ok(xdg) = std::env::var("XDG_CONFIG_HOME") {
        return PathBuf::from(xdg).join("magi");
    }
    std::env::var("HOME").map_or_else(
        |_| PathBuf::from(".magi"),
        |home| PathBuf::from(home).join(".config").join("magi"),
    )
}

fn usage() {
    eprintln!("plug — Entry Plug, o cliente MAGI");
    eprintln!();
    ...
    eprintln!();
    eprintln!("  $MAGI_HOME  onde ficam a identidade e os pins (padrão ~/.config/magi)");
}
...
async fn run_command(runtime: &mut Runtime, client: &mut Client, command: &Command) -> Result<()> {
    match command {
        ...
        Command::About => note(
            runtime,
            format!(
                "MAGI · plug {} · protocolo v{}",
                env!("CARGO_PKG_VERSION"),
                magi_core::PROTOCOL_VERSION
            ),
        ),
