...
//! `specs/06-clientes-gui.md` asked `magi-ffi` for `estado_atual() -> Snapshot`
...
use magi_proto::control::{CageInfo, LineInfo, PilotState};
...
pub struct Notice {
    /// How loud.
    pub severity: magi_proto::control::AlertSeverity,
    /// What about. Enumerated, so each shell writes its own sentence.
    pub reason: magi_proto::control::AlertReason,
...
pub struct Ended {
    /// Enumerated reason.
    pub reason: magi_proto::control::DisconnectReason,
}
...
pub struct Room {
    ...
    pub telemetry: Option<magi_proto::control::Telemetry>,
