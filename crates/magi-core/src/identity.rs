...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        ...
        path.push(format!("magi-identity-test-{name}-{}", std::process::id()));
    ...
    fn a_pin_survives_the_process_that_made_it() {
...

        {
            ...
            store.pin("magi.exemplo", "abc123".into());
        }

        let reopened = FilePinStore::open(path).expect("reopen");
        assert_eq!(reopened.pinned("magi.exemplo"), Some("abc123".into()));
    ...
    fn repinning_a_host_replaces_rather_than_appends() {
        ...
        store.pin("magi.exemplo", "antigo".into());
        store.pin("magi.exemplo", "novo".into());

        let text = std::fs::read_to_string(&path).expect("read");
        assert_eq!(text.lines().count(), 1, "two pins for one host:\n{text}");
        assert_eq!(store.pinned("magi.exemplo"), Some("novo".into()));
    ...
    fn a_corrupt_pin_file_costs_a_repin_and_not_a_failure_to_start() {
        ...
        assert_eq!(store.pinned("magi.exemplo"), None);
