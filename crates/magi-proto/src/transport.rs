...
//! nothing, and ADR 0002 keeps `magi-server` and `magi-core` from sharing a
...
///
/// `specs/04-servidor-magi.md`: "per-sender limit on frames per second (an
...
mod tests {
    ...
    fn the_flood_limit_leaves_room_for_a_hiccup() {
        // specs/04-servidor-magi.md puts an honest client at 50/s. A limit at
...
mod fingerprint_tests {
    ...
    fn the_same_certificate_always_prints_the_same() {
        assert_eq!(
            certificate_fingerprint(b"magi"),
            certificate_fingerprint(b"magi")
        );
    }
