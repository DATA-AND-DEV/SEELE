...
async fn three_clients_in_one_voice_room_hear_each_other() -> Result<()> {
    ...
    for client in [&mut marcela, &mut rafael, &mut carla] {
        client.enter_voice_room(VoiceRoomId(1), None).await?;
    }
...
async fn a_client_without_permission_is_refused() -> Result<()> {
    ...
    observer.enter_voice_room(VoiceRoomId(1), None).await?;
    person.enter_voice_room(VoiceRoomId(1), None).await?;
    tokio::time::sleep(Duration::from_millis(200)).await;
...
async fn media_before_entering_a_voice_room_goes_nowhere() -> Result<()> {
    ...
    inside.enter_voice_room(VoiceRoomId(1), None).await?;
    tokio::time::sleep(Duration::from_millis(200)).await;
