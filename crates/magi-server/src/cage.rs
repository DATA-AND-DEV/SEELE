...
//! # One task per Cage
//!
//! `specs/04-servidor-magi.md`: "one task per **Cage**, owning that Cage's
...
use magi_proto::ids::{CageId, PilotId, Ssrc};
use magi_proto::transport::MAX_FRAMES_PER_SECOND;
use tokio::sync::mpsc;
...
pub struct DropCounts {
    ...
    /// The sender lacks [`magi_proto::control::Permission::Speak`].
...
impl Cage {
    ...
    fn forward(&mut self, from: Ssrc, bytes: &[u8]) {
        ...
        let Ok((header, _payload)) = magi_proto::MediaHeader::decode(bytes) else {
        ...
        // specs/04-servidor-magi.md: a per-sender frames-per-second limit, so a
...
///
/// `specs/04-servidor-magi.md`: one task per Cage, owning its state, reached by
...
mod tests {
    ...
    fn the_payload_is_forwarded_byte_for_byte() {
        // specs/04-servidor-magi.md: "never decodes the Opus". Rewriting even
    ...
    fn a_pilot_without_permission_cannot_speak() {
        // specs/04-servidor-magi.md: "always validate — do not trust the
    ...
    fn a_flood_is_cut_off_at_the_documented_rate() {
        // specs/04-servidor-magi.md: an honest client sends 50/s; above the
    ...
    fn an_honest_sender_is_never_rate_limited() {
        ...
        for seq in 0..magi_proto::transport::NOMINAL_FRAMES_PER_SECOND {
            cage.handle(CageCommand::Datagram {
                from: Ssrc(100),
                bytes: datagram(100, seq as u16),
            });
        ...
        assert_eq!(
            ...
            magi_proto::transport::NOMINAL_FRAMES_PER_SECOND as usize
        );
    }
