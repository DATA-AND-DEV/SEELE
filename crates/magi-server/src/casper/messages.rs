...
use magi_proto::ids::{ClientMessageId, LineId, MessageId, PilotId};
...
impl<'a> Messages<'a> {
    ...
    /// (`specs/04-servidor-magi.md`). The returned messages carry their assigned
    ...
    ///
    /// `specs/04-servidor-magi.md` defaults retention to unlimited, so a
...
mod tests {
    ...
    fn unlimited_retention_prunes_nothing() {
        // specs/04-servidor-magi.md defaults to unlimited. A sweep that deleted
    ...
    fn messages_survive_a_reopen() {
        // specs/04-servidor-magi.md acceptance: "Reinício não perde mensagem
