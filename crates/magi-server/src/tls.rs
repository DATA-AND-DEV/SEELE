...
impl Identity {
    ...
    pub fn fingerprint(&self) -> String {
        self.chain.first().map_or_else(String::new, |certificate| {
            magi_proto::transport::certificate_fingerprint(certificate.as_ref())
        })
    }
}
...
pub fn server_config(identity: Identity) -> Result<quinn::ServerConfig> {
    ...
    transport.max_idle_timeout(Some(
        magi_proto::transport::IDLE_TIMEOUT
            .try_into()
            .context("idle timeout out of range")?,
    ));
    transport.keep_alive_interval(Some(magi_proto::transport::KEEPALIVE));
