//! The `magi-ffi` surface, exercised against a real Dogma.
//!
//! `specs/06-clientes-gui.md` specifies this surface and the rules it must
//! keep. The unit tests in `magi-ffi` pin the shapes; what is here is the part
...
use anyhow::Result;
use magi_ffi::{ConnectConfig, Event, EventListener, Pattern, Plug, PlugError, Snapshot};
...
fn home(name: &str) -> String {
    ...
    path.push(format!("magi-ffi-test-{name}-{}", std::process::id()));
