...
//! `magi-tui`'s own tests, where they run without a network. What is here is
...
async fn a_newcomer_can_hold_a_conversation_with_only_the_help_screen() -> Result<()> {
    ...
    let magi_tui::app::Action::Send(body) = action.expect("nothing to send") else {
    ...
    assert_eq!(
        quit,
        Some(magi_tui::app::Action::Command("q".into())),
