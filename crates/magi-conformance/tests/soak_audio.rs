...
use magi_audio::codec::{VoiceDecoder, VoiceEncoder};
use magi_audio::jitter::{Decision, JitterBuffer, JitterConfig};
use magi_audio::netsim::{talker_stream, ArrivedFrame, Network, NetworkProfile};
...
/// and simulated time is cheap. `MAGI_SOAK_MINUTES` makes it longer when
/// somebody is chasing something that takes an hour to show up.
fn soak_minutes() -> usize {
    std::env::var("MAGI_SOAK_MINUTES")
        .ok()
...
fn ten_minutes_on_a_good_network_does_not_drift_or_starve() {
    ...
    if std::env::var("MAGI_SOAK_TRACE").is_ok() {
        println!(
