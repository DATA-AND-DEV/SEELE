...
//! sound card and no second host — which is why `magi-server` is a library as
...
use magi_core::{Client, MemoryPinStore, Pattern, PinDecision};
use magi_proto::ids::{CageId, Ssrc};
...
fn media(ssrc: Ssrc, seq: u16, payload: &[u8]) -> Vec<u8> {
    ...
    let mut out = vec![0_u8; magi_proto::MAX_DATAGRAM_LEN];
    let len = header
...
async fn three_clients_in_one_cage_hear_each_other() -> Result<()> {
    ...
    // specs/04-servidor-magi.md: forwarded to every *other* subscriber, payload
    // untouched.
...
async fn a_client_without_permission_is_refused() -> Result<()> {
    // The second criterion. specs/04-servidor-magi.md: "always validate — do not
