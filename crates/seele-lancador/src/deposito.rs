...
//!
//! ADR 0026, e o ADR 0046 o mantém sem emenda: *«o pacote é baixado inteiro
...
impl Deposito {
    ...
    /// A instalação desta versão, lida do disco e sem manifesto nenhum.
    ///
    /// # Por que ela existe
    ///
    /// Porque abrir uma versão que **já está instalada** não pode depender da
    /// rede. [`Self::instaladas`] diz quais existem; esta diz o que rodar, e as
    /// duas juntas são o launcher funcionando offline — que é o caso comum, não
    /// o excepcional: quem escolhe hospedar numa versão anterior por causa de um
    /// MOD já a tem no disco.
    ///
    /// `None` quando a versão não está instalada, ou quando o marcador dela é
    /// de antes desta segunda linha existir. **Ausência e não erro**: um
    /// marcador antigo é uma instalação boa cuja procedência do executável
    /// ninguém anotou, e a resposta honesta é «não sei», que quem chama trata
    /// pedindo o manifesto como antes.
    #[must_use]
    pub fn instalacao(&self, versao: &Versao) -> Option<Instalacao> {
        if !self.esta_instalada(versao) {
            return None;
        }
        let raiz = self.pasta_da_versao(versao);
        let marcador = std::fs::read_to_string(raiz.join(MARCADOR)).ok()?;
        let relativo = marcador.lines().nth(1)?.trim();
        let relativo = crate::executavel::CaminhoDoExecutavel::novo(relativo).ok()?;
        let executavel = raiz.join(relativo.como_caminho());
        // Conferido agora, e não quando foi escrito: entre a instalação e este
        // instante alguém pode ter apagado o arquivo. Prometer um caminho que
        // não existe só adia a falha para o `spawn`, onde a mensagem é do
        // sistema operacional e não do produto.
    ...
    fn preparar_e_publicar(
        ...
        //
        // **Duas linhas, e a segunda é o que torna o depósito utilizável sem o
        // manifesto.** Ele guardava só a versão, e com isso a pergunta «qual
        // executável é o desta instalação?» só tinha resposta com o manifesto na
        // mão — que vem da rede. Um launcher que precisa da rede para abrir uma
        // versão **já instalada** não é um launcher: é um atualizador.
        //
        // A ordem importa e é a de sempre: versão primeiro, porque é o que já
        // estava lá e é o que um marcador antigo tem sozinho. Quem lê trata a
        // ausência da segunda linha como «não sei», e não como erro.
        std::fs::write(
            preparo.join(MARCADOR),
            format!(
                "{}\n{}\n",
                publicacao.version.como_texto(),
                executavel_relativo.como_texto()
            ),
        )
        .map_err(em_falha)?;
...
mod testes {
    ...
    /// O depósito sabe dizer o que rodar, sem manifesto e sem rede.
    ///
    /// É a diferença entre um launcher e um atualizador: abrir uma versão que
    /// já está no disco não pode depender de buscar nada. Quem escolhe hospedar
    /// numa versão anterior por causa de um MOD já a tem instalada.
    #[test]
    fn o_deposito_diz_qual_e_o_executavel_de_uma_versao_instalada() {
        let raiz = pasta("instalacao-se-descreve");
        let deposito = Deposito::em(&raiz);
        let versao = Versao::nova("1.0.0-teste").unwrap();
        instalar_uma(&deposito, "1.0.0-teste");
        ...
        let instalacao = deposito
            .instalacao(&versao)
            .expect("uma versão instalada tem de saber dizer o que rodar");
        assert_eq!(instalacao.versao, versao);
        assert_eq!(
            instalacao.executavel,
            deposito.pasta_da_versao(&versao).join(EXECUTAVEL),
            "o executável apontado não é o que a instalação publicou"
        );
        assert!(
            instalacao.executavel.is_file(),
            "o caminho devolvido não existe em disco"
        );
        let _ = std::fs::remove_dir_all(&raiz);
    }

    /// Um marcador de antes desta segunda linha é «não sei», e não um erro.
    #[test]
    fn um_marcador_sem_a_linha_do_executavel_responde_que_nao_sabe() {
        let raiz = pasta("marcador-antigo");
        let deposito = Deposito::em(&raiz);
        let versao = Versao::nova("1.0.0-teste").unwrap();
        instalar_uma(&deposito, "1.0.0-teste");

        // O formato antigo: só a versão.
        std::fs::write(
            deposito.pasta_da_versao(&versao).join(MARCADOR),
            b"1.0.0-teste\n",
        )
        .unwrap();

        assert!(
            deposito.esta_instalada(&versao),
            "a instalação continua boa: o que falta é a anotação, não os bytes"
        );
        assert_eq!(
            deposito.instalacao(&versao),
            None,
            "um marcador antigo tem de responder «não sei», para quem chama \
             voltar a pedir o manifesto — e não passar por instalação quebrada"
        );
        let _ = std::fs::remove_dir_all(&raiz);
    }

    /// E o executável que sumiu depois de instalado também é «não sei».
    #[test]
    fn um_executavel_apagado_depois_da_instalacao_nao_e_prometido() {
        ...
        assert_eq!(
            deposito.instalacao(&versao),
            None,
            "prometer um caminho que não existe só adia a falha para o `spawn`, \
             onde a mensagem é do sistema operacional e não do produto"
        );
        let _ = std::fs::remove_dir_all(&raiz);
    }
