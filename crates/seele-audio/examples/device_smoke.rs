...
//! checklist, and the only way [`seele_audio::device`] gets exercised at all.
//!
...
//!
//! Run with: `cargo run --package seele-audio --example device_smoke`
...
fn run() -> Result<(), DeviceError> {
    ...
    if io.capture_rate_hz != seele_audio::SAMPLE_RATE_HZ
        || io.playback_rate_hz != seele_audio::SAMPLE_RATE_HZ
    {
        println!(
            "note: a device is not at {} Hz. Resampling is mandatory and is task M1.4.",
            seele_audio::SAMPLE_RATE_HZ
        );
    }
    ...
    let mut to_pipeline = match RateConverter::new(io.capture_rate_hz, seele_audio::SAMPLE_RATE_HZ)
    {
    ...
    let mut to_device = match RateConverter::new(seele_audio::SAMPLE_RATE_HZ, io.playback_rate_hz) {
        Ok(converter) => converter,
        Err(error) => {
