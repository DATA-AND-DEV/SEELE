...
//! listener.
//!
//! Run with: `cargo run --release --package seele-audio --example jitter_profiles`

use seele_audio::jitter::{Decision, JitterBuffer, JitterConfig};
use seele_audio::netsim::{run, NetworkProfile};
...
fn main() {
    ...
    let frames = seele_audio::netsim::talker_stream(25, 50, 40);
    let mut buffer: JitterBuffer<u16> = JitterBuffer::new(JitterConfig::default());
    let mut clock_ms = 0.0_f64;
    let mut index = 0;
    for _ in 0..3_200 {
