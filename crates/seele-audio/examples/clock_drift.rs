...
//! [`seele_audio::resample::RateConverter::adjust_ratio`] does to the stream, so
//! the depth trend below is the trend the real pipeline will show.
//!
//! Run with: `cargo run --release --package seele-audio --example clock_drift`

use seele_audio::drift::DriftTracker;
use seele_audio::jitter::{Decision, JitterBuffer, JitterConfig};
use seele_audio::netsim::{drifting_stream, Network, NetworkProfile};
