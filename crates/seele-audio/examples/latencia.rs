...
//! cargo run --release --example latencia -p seele-audio -- --repeticoes 40
//! ```
//!
//! # Two ways to run it
//!
...
//! resampling in [`seele_audio::resample`]. Repeatable to a sample, and the
//! number to optimise.
//!
...
use seele_audio::latency::{click, find_delay, Report};
use seele_audio::{device, SAMPLE_RATE_HZ};
...
fn main() {
    ...
    println!("SEELE · medição de latência de ida e volta");
    println!();
