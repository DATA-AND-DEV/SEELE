...
//! implementado em `magi-core` e aparece nas duas interfaces." Nothing in this
//! crate decides anything. It owns a thread, a connection and a
//! [`magi_core::Room`], and it turns method calls into commands and server
...
impl Plug {
    ...
    fn audio_state(&self) -> (bool, VoiceMode, bool, bool, bool, f32, bool, u32) {
        ...
        let mode = match voice.mode() {
            magi_core::VoiceMode::PushToTalk => VoiceMode::PushToTalk,
            magi_core::VoiceMode::VoiceActivated => VoiceMode::VoiceActivated,
            magi_core::VoiceMode::Open => VoiceMode::Open,
        };
        (
            true,
            mode,
...
fn resolve(target: &str) -> Result<(SocketAddr, String), PlugError> {
    let with_port = if target.contains(':') {
        ...
        format!("{target}:{}", magi_core::DEFAULT_PORT)
    };
    let host = with_port
        .rsplit_once(':')
...
fn fold(shared: &Arc<Shared>, message: &magi_core::ServerMessage) {
...
/// error message.
fn classify_connect_failure(error: &magi_core::ConnectError) -> PlugError {
    match error {
        magi_core::ConnectError::LocalEndpoint | magi_core::ConnectError::Unreachable => {
            PlugError::Unreachable
        }
        magi_core::ConnectError::TlsRefused | magi_core::ConnectError::ProtocolViolation => {
            PlugError::Refused {
                reason: EndReason::Incompatible,
            }
        }
        magi_core::ConnectError::PinChanged { pinned, offered } => PlugError::PinChanged {
            pinned: pinned.clone(),
            offered: offered.clone(),
        },
        magi_core::ConnectError::HandshakeTimeout => PlugError::HandshakeTimeout,
        magi_core::ConnectError::Refused { reason } => PlugError::Refused {
            reason: (*reason).into(),
        },
    }
}
...
mod tests {
    ...
    fn the_snapshot_marks_which_pilot_is_us() {
        ...
        use magi_core::{CageInfo, PilotId, PilotProfile, ServerMessage, SessionId, Ssrc};
    ...
    fn a_listener_hears_what_the_room_folded() {
        use magi_core::{AlertReason, AlertSeverity, ServerMessage};
