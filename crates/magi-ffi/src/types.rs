...
//! enums. **No `magi-proto` newtype appears in this module**, and that is the
...
impl From<VoiceMode> for magi_core::VoiceMode {
...
impl From<magi_core::SyncBand> for SyncBand {
    fn from(band: magi_core::SyncBand) -> Self {
        match band {
            magi_core::SyncBand::Nominal => Self::Nominal,
            magi_core::SyncBand::Acceptable => Self::Acceptable,
            magi_core::SyncBand::Degraded => Self::Degraded,
            magi_core::SyncBand::Critical => Self::Critical,
        }
    }
}
...
impl From<magi_core::AlertSeverity> for Severity {
    fn from(severity: magi_core::AlertSeverity) -> Self {
        match severity {
            magi_core::AlertSeverity::Info => Self::Info,
            magi_core::AlertSeverity::Warning => Self::Warning,
            magi_core::AlertSeverity::Critical => Self::Critical,
        }
    }
}
...
impl From<magi_core::AlertReason> for NoticeReason {
    fn from(reason: magi_core::AlertReason) -> Self {
        match reason {
            magi_core::AlertReason::Mentioned => Self::Mentioned,
            magi_core::AlertReason::SubsystemChanged => Self::SubsystemChanged,
            magi_core::AlertReason::SyncDegraded => Self::SyncDegraded,
            magi_core::AlertReason::CageEntryRefused => Self::CageEntryRefused,
            magi_core::AlertReason::PermissionDenied => Self::PermissionDenied,
            magi_core::AlertReason::CageFull => Self::CageFull,
            magi_core::AlertReason::OperatorNotice => Self::OperatorNotice,
        }
    }
}
...
impl From<magi_core::DisconnectReason> for EndReason {
    fn from(reason: magi_core::DisconnectReason) -> Self {
        match reason {
            magi_core::DisconnectReason::Incompatible => Self::Incompatible,
            magi_core::DisconnectReason::CredentialRejected => Self::CredentialRejected,
            magi_core::DisconnectReason::HandshakeTimeout => Self::HandshakeTimeout,
            magi_core::DisconnectReason::Kicked => Self::Kicked,
            magi_core::DisconnectReason::Banned => Self::Banned,
            magi_core::DisconnectReason::DogmaFull => Self::DogmaFull,
            magi_core::DisconnectReason::ScheduledMaintenance => Self::ScheduledMaintenance,
            magi_core::DisconnectReason::ServerShuttingDown => Self::ServerShuttingDown,
            magi_core::DisconnectReason::Timeout => Self::Timeout,
            magi_core::DisconnectReason::ProtocolViolation => Self::ProtocolViolation,
            magi_core::DisconnectReason::RateLimited => Self::RateLimited,
        }
    }
}
...
pub struct Message {
    ...
    ///
    /// The unit is in the name deliberately — see `magi-proto`, where getting
...
mod tests {
    ...
    fn every_disconnect_reason_the_protocol_has_maps_to_one_here() {
        ...
        let all = [
            magi_core::DisconnectReason::Incompatible,
            magi_core::DisconnectReason::CredentialRejected,
            magi_core::DisconnectReason::HandshakeTimeout,
            magi_core::DisconnectReason::Kicked,
            magi_core::DisconnectReason::Banned,
            magi_core::DisconnectReason::DogmaFull,
            magi_core::DisconnectReason::ScheduledMaintenance,
            magi_core::DisconnectReason::ServerShuttingDown,
            magi_core::DisconnectReason::Timeout,
            magi_core::DisconnectReason::ProtocolViolation,
            magi_core::DisconnectReason::RateLimited,
        ];
    ...
    fn every_alert_reason_maps_to_one_of_its_own() {
        let all = [
            magi_core::AlertReason::Mentioned,
            magi_core::AlertReason::SubsystemChanged,
            magi_core::AlertReason::SyncDegraded,
            magi_core::AlertReason::CageEntryRefused,
            magi_core::AlertReason::PermissionDenied,
            magi_core::AlertReason::CageFull,
            magi_core::AlertReason::OperatorNotice,
        ];
    ...
    fn the_bands_agree_with_the_core() {
        ...
        for ratio in 0..=100u8 {
            let band: SyncBand = magi_core::SyncBand::of(ratio).into();
