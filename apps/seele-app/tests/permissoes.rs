...
// **No Windows este arquivo não compila nada, e não é preguiça.**
//
// O binário destes testes nem chega a rodar lá: `STATUS_ENTRYPOINT_NOT_FOUND`,
// 0xc0000139 — o carregador do Windows achando uma DLL e não achando um export
// dentro dela. Diagnosticado na máquina: não há `WebView2Loader.dll` ao lado do
// executável nem no PATH, e o `webview2-com-sys` a deixa no diretório de build
// em três arquiteturas sem copiar nenhuma. É montagem de toolchain e não código
// daqui — o app empacotado roda naquela máquina todos os dias.
//
// `#[ignore]` foi tentado primeiro e **não serve**: o binário falha ao carregar,
// antes de qualquer atributo de teste ser lido, e a bateria inteira do Windows
// abortava ali. Só não linkar o Tauri resolve.
//
// A cobertura não se perde: o que estes guardas provam é a ACL, que sai do
// `tauri.conf.json` e das capacidades geradas — arquivos iguais nos três
// sistemas. Rodar em dois prova a invariante inteira.
//
// E a dívida não fica invisível, que é a lição desta bateria: o teste logo
// abaixo existe só para aparecer no relatório do Windows dizendo, pelo nome, que
// aqui há algo que não roda. Um `cfg` mudo teria a mesma cor de passar.
#[cfg(not(windows))]
mod guardas {
...
/// No Windows os guardas acima não rodam, e isto aparece no relatório dizendo-o.
#[cfg(windows)]
#[test]
fn no_windows_estes_guardas_nao_rodam() {
    eprintln!(
        "PARCIAL: os guardas da ACL não rodam no Windows — o binário de teste do \
         Tauri não carrega ali (STATUS_ENTRYPOINT_NOT_FOUND). Ver a nota no topo \
         deste arquivo. Eles rodam no macOS e no Linux, sobre os mesmos arquivos."
    );
}
