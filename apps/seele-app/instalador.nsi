; ============================================================================
; O instalador do SEELE — bifurcado do modelo do `tauri-bundler`.
;
; ORIGEM: tauri-bundler 2.9.4, src/bundle/windows/nsis/installer.nsi
;
; ---- por que bifurcar ----
;
; O modelo do Tauri desenha as páginas padrão do NSIS. O SEELE tem uma
; identidade visual própria — `apps/seele-app/ui/tokens.css`, ADR 0014 — e o
; instalador é a primeira tela que alguém vê do produto. As páginas próprias que
; o desenho pede não se alcançam por configuração: `installerHooks` roda **dentro**
; das seções, e não substitui página nenhuma.
;
; ---- o que NÃO pode mudar aqui ----
;
; **O modo silencioso.** Este mesmo `.exe` é o que o atualizador do Tauri baixa e
; roda com `/S` para aplicar uma atualização. Toda página passa por
; `SkipIfPassive`, e uma página nova que esqueça isso trava a atualização
; automática de todo mundo — numa tela que ninguém está vendo.
;
; **Os ganchos.** `NSIS_HOOK_PREINSTALL`, `NSIS_HOOK_POSTINSTALL` e
...
;
; Um modelo bifurcado não recebe as correções de quem o escreveu. Quando o
; `tauri-bundler` subir de versão, as mudanças dele **não** chegam aqui sozinhas,
; e uma delas pode ser justamente a que conserta a instalação em algum Windows
; que ninguém aqui tem.
;
; Por isso a versão de origem está escrita acima e um guarda em
; `apps/seele-app/tests/empacotamento.rs` compara essa linha com a versão que o
; `Cargo.lock` de fato resolve. Quando elas divergirem, o guarda reprova e diz
; para rebifurcar: comparar o modelo novo com este e trazer o que mudou.
; ============================================================================
