...
//! `design/magi-tokens.css` rather than something a build step produces. A copy
...
fn repo_root() -> PathBuf {
    // `CARGO_MANIFEST_DIR` is `apps/magi-app`.
...
fn the_served_tokens_are_the_frozen_tokens() {
    let root = repo_root();
    let frozen = std::fs::read_to_string(root.join("design/magi-tokens.css"))
        .expect("design/magi-tokens.css is the source of truth and must exist");
    let served = std::fs::read_to_string(root.join("apps/magi-app/ui/tokens.css"))
        .expect("apps/magi-app/ui/tokens.css is what the window loads");

    assert_eq!(
        served, frozen,
        "the desktop client is serving different design tokens than the frozen ones.\n\
         Copy design/magi-tokens.css over apps/magi-app/ui/tokens.css — and if the \
...
fn the_stylesheet_uses_no_colour_the_tokens_do_not_define() {
    ...
    let css = std::fs::read_to_string(repo_root().join("apps/magi-app/ui/magi.css"))
