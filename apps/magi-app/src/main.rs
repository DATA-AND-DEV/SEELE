//! The desktop client — a Tauri shell over [`magi_ffi`].
//!
...
//! judgement, it belongs in `magi-core`, and the terminal client would have had
//! to grow the same one.
//!
//! # Threading
//!
//! [`magi_ffi::Plug::connect`] blocks, so it runs on a blocking thread. Events
...
use magi_ffi::{ConnectConfig, Event, EventListener, Plug, PlugError, Snapshot, VoiceMode};
...
const EVENT_CHANNEL: &str = "magi://event";
...
/// configuration and the core knows how to persist an identity. `$MAGI_HOME`
...
fn config_dir(app: &AppHandle) -> String {
    if let Ok(home) = std::env::var("MAGI_HOME") {
        return home;
    }
    // The same `~/.config/magi` the terminal client uses, deliberately: two
    // clients on one machine should be one pilot unless told otherwise.
    if let Ok(xdg) = std::env::var("XDG_CONFIG_HOME") {
        return format!("{xdg}/magi");
    }
    if let Ok(home) = std::env::var("HOME") {
        return format!("{home}/.config/magi");
    }
    app.path()
        .app_config_dir()
        .map(|path| path.to_string_lossy().into_owned())
        .unwrap_or_else(|_| ".magi".to_owned())
}
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "magi_app=info,magi_ffi=info,magi_core=info".into()),
        )
        .init();
